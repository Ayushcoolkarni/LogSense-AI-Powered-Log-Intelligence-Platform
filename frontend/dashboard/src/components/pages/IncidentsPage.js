...
export default function IncidentsPage() {
  ...
  // Pre-populate filters from URL query params (set by StatCard clicks)
  const [search,   setSearch]   = useState('');
  const [severity, setSeverity] = useState(searchParams.get('severity') || '');
  const [status,   setStatus]   = useState(searchParams.get('status')   || '');
  const [page,     setPage]     = useState(0);

  // If URL params change (e.g. browser back/forward), sync filters
  useEffect(() => {
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 24, background: '#0f172a' }}>
        ...
        {activeFilter && (
          <div style={{
            ...
            <span style={{ color: '#93c5fd', fontSize: 13 }}>Filtered by: <strong>{activeFilter}</strong></span>
            <button
              onClick={() => { setSeverity(''); setStatus(''); setPage(0); }}
              style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: 13, marginLeft: 'auto' }}
            >
              ✕ Clear filter
            </button>
          </div>
            ...
            <tbody>
              {loading && incidents.length === 0 ? (
                ...
                >
                  ...
                  <td style={{ padding: '12px 16px', color: '#94a3b8' }}>{(inc.anomalyType || '').replace(/_/g, ' ')}</td>
                  <td style={{ padding: '12px 16px' }}><Badge label={inc.severity} /></td>
                  <td style={{ padding: '12px 16px' }}><Badge label={inc.status} /></td>
                  <td style={{ padding: '12px 16px', color: '#64748b' }}>
                    {inc.detectedAt ? formatDistanceToNow(parseISO(inc.detectedAt), { addSuffix: true }) : '—'}
                  </td>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 20 }}>
            ...
            <span style={{ color: '#64748b', fontSize: 13, alignSelf: 'center' }}>{page + 1} / {totalPages}</span>
